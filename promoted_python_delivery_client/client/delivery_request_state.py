...
class DeliveryRequestState:
    ...
    def get_request_to_send(self, max_request_insertions: int) -> Request:
        ...
        if req.insertion is not None and len(req.insertion) > max_request_insertions:
            req.insertion = req.insertion[0:max_request_insertions]
        if req.insertion_matrix is not None and len(req.insertion_matrix) > max_request_insertions:
